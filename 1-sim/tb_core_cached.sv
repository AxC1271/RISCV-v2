...
module core_riscv_tb();
    ...
    initial begin
        ...
        if (fail_count == 0)
            ...
            $display("SOME TESTS FAILED -- check pipeline/cache hit/stall logic\n");
        $finish;
    end

    initial begin
        #10000000;
        $display("[TIMEOUT] Simulation exceeded time limit -- check for unresolved cache stall/deadlock");
        $fatal;
    end
    ...
    initial begin
        if ($test$plusargs("verbose")) begin
            ...
            forever begin
                @(posedge clk);
                $display("[T=%0t] | PC=%08h (I$:rdy=%b,mem_req=%b) | ID:%08h | EX:rd=x%0d | MEM:(D$:rdy=%b,mrd=%b,mwr=%b) | WB:rd=x%0d(vld=%b) | Stalls:[lu=%b f_stl=%b m_stl=%b]",
                    $time,
                    dut.pc_current, dut.icache_ready, dut.imem_req,
                    dut.id_instr,
                    dut.ex_rd,
                    dut.dcache_ready, dut.mem_memread, dut.mem_memwrite,
                    dut.wb_rd, dut.wb_valid,
                    dut.load_use_stall, dut.fetch_stall, dut.mem_stall
                );
            end
        end
    end
