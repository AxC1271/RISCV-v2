module data_cache # (
    ...
    logic[SET_INDEX_BITS] set_index;
    ...
    assign cache_ready = (state == IDLE && (rd_en || wr_en) && hit) || cache_ready_r;
    ...
    always_comb begin
        if (state == IDLE && hit && rd_en)
            rd_state = data_array[set_index][hit_way][word_offset];
        else
            rd_data = rd_data_r;
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            miss_wr_data <= '0;
            cache_ready_r <= 1'b0;
            for (int i = 0; i < BLOCK_WORDS; i++) refill_data <= '0;
            ...
            case (state)
                IDLE: begin
                    if (rd_en || wr_en) begin
                        if (hit) begin
                            ...
                            cache_ready_r <= 1'b1;
                            rd_data_r <= wr_en ? wr_data : data_array[set_index][hit_way][word_offset];
                            ...
                            word_counter <= '0;
                            if (valid_array[set_index][replace_way] && dirty_array[set_index][replace_way])
                                state <= WRITEBACK;
                            else 
                                state <= ALLOCATE;
                        end
                ...
                ALLOCATE: begin
                    if (mem_ready) begin
                        ...
                        if (word_counter == (BLOCK_WORDS-1)) begin
                            if (miss_wr_en) rd_data_r <= miss_wr_data;
