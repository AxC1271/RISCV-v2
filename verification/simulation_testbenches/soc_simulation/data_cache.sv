...
module data_cache # (
    ...
    // address decode (combinational, live inputs)
            ...
            case (state)
                IDLE: begin
                    if (rd_en || wr_en) begin
                        if (hit) begin
                            // hit — complete in one cycle, pulse ready immediately
                            if (wr_en) begin
                            ...
                            ready               <= 1'b1; // pulse ready, stay in IDLE
                            // no state change — back to IDLE next cycle naturally
                        end else begin
                            // miss — latch request and choose victim
                ...
                ALLOCATE: begin
                    if (mem_ready) begin
                        // write incoming word (or store data for writes)
                        ...
                        if (word_counter == WORD_OFF_BITS'(BLOCK_WORDS - 1)) begin
                            ...
                            ready               <= 1'b1; // pulse ready, return to IDLE
                            state               <= IDLE;
                        end else begin
                            word_counter <= word_counter + 1'b1;
                        end
