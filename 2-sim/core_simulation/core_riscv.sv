...
module core_riscv # (
    ...
    parameter ICACHE_SETS       = 64,
    parameter ICACHE_LINE_WORDS = 4,
    parameter DCACHE_SIZE       = 4096,
    parameter DCACHE_BLOCK      = 16,
    ...
    instr_cache # (
        .NUM_SETS       (ICACHE_SETS),
        .WORDS_PER_LINE (ICACHE_LINE_WORDS)
    ) icache (
        .clk         (clk),
        .rst_n       (rst_n),
        .cpu_addr    (pc_current),
    ...
    // zero out register specifiers that aren't real operands so the hazard
    // and forward units don't act on immediate bits (e.g. JAL's rs1 field)
    ...
    // operand muxes: op_a covers rs1 / pc (AUIPC) / zero (LUI)
    ...
    // branch resolution -- forwarded operands, same as the ALU sees
    logic        branch_taken;
    ...
    // link value for JAL/JALR: jump is fully consumed here
    ...
    assign periph_wdata = dcache_wdata;  
